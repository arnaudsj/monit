...
form: name@domain. The keyword I<subject:> is for the email
...
poll-cycles. For example, if a service had I<x> restarts within
I<y> poll-cycles (where I<x> <= I<y>) then Monit will timeout and
...
=item IF NUMBER RESTART NUMBER CYCLE(S) THEN TIMEOUT
...
NB! a Monit client will use the I<first> username:password pair
in an allow list and you should B<not> define the first user as a
...
=item 1. CHECK PROCESS <unique name> PIDFILE <path>
...
I<basedir>, I<slot(s)>, I<system>, I<idfile>, I<gps>, I<rsync>
and I<failed>

And here is a complete list of B<noise keywords> ignored by
monit:

I<is>, I<as>, I<are>, I<on(ly)>, I<with(in)>, I<and>, I<has>,
...
Finally an example with all statements:
 ...
 check process radiusd with pidfile /home/aland/git/2_1_x/raddb/radiusd.pid
       start program = "/etc/init.d/freeradius start"
       stop program = "/etc/init.d/freeradius stop"
       if failed host 127.0.0.1 port 2000 type udp protocol radius secret testing1234 then alert
       if 5 restarts within 5 cycles then timeout
