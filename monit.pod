...
number of poll-cycles. For example, if a service had I<x> restarts
within I<y> poll-cycles (where I<x> <= I<y>) then Monit will perform
custom action (for example unmonitor the service). If a timeout occurs
...
Here is an example where Monit will unmonitor the if it was restarted
...
the content of host option which specifies the target host to connect to is used.
...
I<secret>, I<target>, I<maxforward>, I<hostheader> and I<failed>
...
Copyright (C) 2010 by Tildeslash Ltd. All Rights Reserved. This
